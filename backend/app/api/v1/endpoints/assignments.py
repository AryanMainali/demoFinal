...
def publish_assignment(
    ...
    if not was_published:
        recipient_ids = set(
            get_active_student_ids_for_course(db, assignment.course_id)
            + get_assistant_ids_for_course(db, assignment.course_id)
        )
        if recipient_ids:
            notify_users(
                db,
                ...
                title=f"New assignment published: {assignment.title}",
                message=f"A new assignment is now available in {assignment.course.code}: {assignment.title}",
