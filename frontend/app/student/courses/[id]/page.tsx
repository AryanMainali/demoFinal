    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-50/50">
            ...
            <main className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10">
                ...
                {/* RESOURCES TAB */}
                {activeTab === 'resources' && (() => {
                    const withResources = courseAssignments.filter(a => a.video_url);
                    return (
                        <div className="space-y-4">
                            {withResources.length === 0 ? (
                                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-20 text-center">
                                    <LibraryBig className="w-12 h-12 text-gray-200 mx-auto mb-3" />
                                    <p className="font-medium text-gray-500">No study resources yet</p>
                                    <p className="text-sm text-gray-400 mt-1">Your instructor hasn't linked any resources yet.</p>
                                </div>
                            ) : (
                                withResources.map(a => {
                                    const url = a.video_url!;
                                    const ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&\s?#]+)/);
                                    const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
                                    const loomMatch = url.match(/loom\.com\/share\/([a-zA-Z0-9]+)/);
                                    const isEmbed = ytMatch || vimeoMatch || loomMatch;
                                        ...
                                        <div key={a.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                                            ...
                                            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <div className="w-9 h-9 rounded-xl bg-[#862733]/10 flex items-center justify-center shrink-0">
                                                        <FileCode className="w-4 h-4 text-[#862733]" />
                                                    </div>
                                                    <div className="min-w-0">
                                                        <p className="font-semibold text-gray-900 truncate text-sm">{a.title}</p>
                                                        {a.due_date && (
                                                            <p className="text-xs text-gray-400 mt-0.5">Due {format(new Date(a.due_date), 'MMM d, yyyy')}</p>
                                                        )}
                                                    </div>
                                                </div>
                                                <a
                                                    ...
                                                    className="shrink-0 flex items-center gap-1.5 text-xs font-medium text-[#862733] hover:underline ml-3"
                                                >
                                            ...
                                            {isEmbed ? (
                                                ...
                                                >
                                                    <div className="w-12 h-12 rounded-xl bg-gray-50 group-hover:bg-[#862733]/5 flex items-center justify-center shrink-0 transition-colors">
                                                        <BookOpen className="w-6 h-6 text-gray-400 group-hover:text-[#862733] transition-colors" />
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-sm font-medium text-gray-800 truncate">{url}</p>
                                                        <p className="text-xs text-gray-400 mt-0.5">Click to open resource</p>
                                                    </div>
                                                    <ExternalLink className="w-4 h-4 text-gray-300 group-hover:text-[#862733] shrink-0 transition-colors" />
                                                </a>
                                            )}
                                        </div>
                                    );
